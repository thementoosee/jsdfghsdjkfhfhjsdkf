...
interface EventSubStatus {
  ...
  chat_subscription_status?: string | null;
  missing_chat_scopes?: string[];
}

interface TipsSourceStatus {
  configured: boolean;
  se_jwt_available?: boolean;
  se_account_id_available?: boolean;
}
...
export function TwitchIntegration() {
  ...
  const [tipsStatus, setTipsStatus] = useState<TipsSourceStatus | null>(null);
  ...
  async function handleOAuthCallback(oauthCode: string) {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('twitch-oauth-callback', {
        body: { code: oauthCode, redirect_uri: redirectUri },
      });
      ...
      const result = data as {
        ...
        channel_name?: string;
        scopes?: string[];
        setup?: { created?: string[]; chat_ingest_status?: ChatIngestStatus };
      };
      console.log('[OAuth] Result success:', Boolean(result?.success), 'scopes_count:', result?.scopes?.length ?? 0);
      ...
      const linkedChannel = result.channel_name || DEFAULT_CHANNEL;
      const chatStatus = result?.setup?.chat_ingest_status || 'offline';
      setStatusMessage(
        `OAuth OK · Canal: ${linkedChannel} · Chat ingest: ${chatStatus} · Subscriptions: ${created}`
      );
  ...
  async function loadStoredConfig() {
    try {
      ...
      if (data) {
        const channel = data.channel_name || DEFAULT_CHANNEL;
  ...
  async function loadEventSubStatus() {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('streamelements-socket', {
        body: { action: 'status' },
      });
      if (error) throw error;
      setTipsStatus((data as TipsSourceStatus) || { configured: false });
    } catch (error) {
      console.warn('StreamElements tips status unavailable:', error);
      setTipsStatus({ configured: false });
    }
  }
  ...
  async function activateEventSub() {
    ...
    try {
      // Prefer tokens already saved by OAuth in twitch_config (24/7 path).
      if (!token && status?.configured && status?.token_valid) {
        const { data, error } = await supabase.functions.invoke('twitch-eventsub', {
          body: { action: 'setup' },
        });
        if (error) throw error;
        ...
        if (result?.error) throw new Error(result.error);
        const created = result?.created?.length ? result.created.join(', ') : 'none';
        setStatusMessage(`EventSub reativado! Subscriptions: ${created}`);
        await loadEventSubStatus();
        await loadRecentAlerts();
        return;
      }
      ...
      if (!token) {
        setStatusError('Usa "Entrar com Twitch" — o access token fica guardado automaticamente. Nao precisas de o colar.');
        return;
      }
  ...
  function loginWithTwitch() {
    try {
      ...
      const redirectUri = `${window.location.origin}/`;
      const oauthUrl = `https://id.twitch.tv/oauth2/authorize?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=${encodeURIComponent(OAUTH_SCOPES)}`;

      console.log('[OAuth] Redirecting to Twitch authorize');
  ...
  async function insertTestRecentEvent(
    type: 'follow' | 'subscription' | 'resubscription' | 'gift_subscription' | 'raid' | 'cheer' | 'tip'
  ) {
    setStatusError(null);
    setStatusMessage(null);
    const stamp = Date.now();
    const eventId = `test_${type}_${stamp}`;

    try {
      if (type === 'tip') {
        const { error } = await supabase.from('streamelements_events').insert({
          event_id: eventId,
          event_type: 'tip',
          ...
          months: 0,
          message: 'Test tip',
          raw_data: { test_event: true, currency: 'EUR' },
        });
        if (error) throw error;
      } else {
        const viewers = type === 'raid' ? 42 : 0;
        const amount = type === 'cheer' ? 100 : type === 'gift_subscription' ? 1 : 0;
        const months = type === 'resubscription' ? 3 : type === 'subscription' ? 1 : 0;
        const { error } = await supabase.from('twitch_alerts').insert({
          event_id: eventId,
          alert_type: type,
          username: `test_${type}`,
          display_name: `Test ${type}`,
          amount: type === 'raid' ? 0 : amount,
          months,
          message: type === 'cheer' ? 'Test cheer' : null,
          metadata: {
            test_event: true,
      ...
      setStatusMessage(`Evento de teste inserido: ${type} (${eventId})`);
      await loadRecentAlerts();
    } catch (error) {
      const msg = error instanceof Error ? error.message : 'Falha ao inserir evento de teste';
      setStatusError(msg);
    }
  }

  async function clearTestRecentEvents() {
    if (!confirm('Apagar apenas eventos marcados como test_event / event_id test_* ?')) return;
    setStatusError(null);
    try {
      const { error: twitchError } = await supabase
        .from('twitch_alerts')
        .delete()
        .like('event_id', 'test_%');
      ...
      setStatusMessage('Eventos de teste removidos');
      await loadRecentAlerts();
    } catch (error) {
      const msg = error instanceof Error ? error.message : 'Falha ao limpar testes';
      setStatusError(msg);
    }
  }

  async function clearAlerts() {
    if (!confirm('Limpar todos os alertas Twitch?')) return;
        ...
        <div className="p-6 space-y-4">
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm text-blue-300">
                <p className="font-semibold uppercase mb-2">Chat 24/7 via EventSub</p>
                <ol className="list-decimal list-inside space-y-1">
                  <li>Clica em Entrar com Twitch e autoriza (inclui scopes de chat)</li>
                  <li>EventSub cria a sub channel.chat.message no webhook</li>
                  <li>Chat ingest fica online só quando essa sub estiver enabled</li>
                </ol>
              </div>
            </div>
          </div>
          ...
          {status && (
            <div className="bg-gray-800/60 border border-gray-700 rounded-lg p-4 text-sm text-gray-300 space-y-1">
              <p>
                <strong>Twitch EventSub (alerts):</strong>{' '}
                <span className={(status.configured && status.token_valid) ? 'text-green-400 font-bold uppercase' : 'text-gray-400 font-bold uppercase'}>
                  {(status.configured && status.token_valid) ? 'online' : 'offline'}
                </span>
              </p>
              <p>
                <strong>Chat ingest:</strong>{' '}
                <span
                  className={
                    status.chat_ingest_status === 'online'
                      ? 'text-green-400 font-bold uppercase'
                      : status.chat_ingest_status === 'reconnecting'
                        ? 'text-yellow-400 font-bold uppercase'
                        : status.chat_ingest_status === 'erro'
                          ? 'text-red-400 font-bold uppercase'
                          : 'text-gray-400 font-bold uppercase'
                  }
                >
                  {status.chat_ingest_status || 'offline'}
                </span>
              </p>
              <p>
                <strong>StreamElements tips:</strong>{' '}
                <span className={tipsStatus?.configured ? 'text-green-400 font-bold uppercase' : 'text-yellow-400 font-bold uppercase'}>
                  {tipsStatus?.configured ? 'online' : 'offline'}
                </span>
                {!tipsStatus?.configured && (
                  <span className="text-yellow-400/80"> — tips indisponíveis até configurar SE</span>
                )}
              </p>
              <p><strong>Chat subscription:</strong> {status.chat_subscription_status || 'none'}</p>
              ...
              {status.missing_chat_scopes && status.missing_chat_scopes.length > 0 && (
                <p className="text-red-400">
                  <strong>Scopes em falta:</strong> {status.missing_chat_scopes.join(', ')}
                </p>
              )}
            </div>
          )}
          ...
          <div className="bg-gray-800/40 border border-gray-700 rounded-lg p-4 space-y-3">
            <p className="text-sm font-semibold text-gray-200 uppercase">Modo teste — Eventos recentes</p>
            <p className="text-xs text-gray-400">
              Insere eventos com <code className="text-gray-300">event_id=test_*</code> e <code className="text-gray-300">test_event: true</code> (sem custo / sem SE pago).
            </p>
            <div className="flex flex-wrap gap-2">
              {([
                ...
                <button
                  key={type}
                  type="button"
                  onClick={() => void insertTestRecentEvent(type)}
                  className="px-3 py-2 bg-gray-700 hover:bg-gray-600 text-gray-100 rounded text-xs font-bold uppercase"
                >
                  {type}
                </button>
              ))}
              <button
                type="button"
                onClick={() => void clearTestRecentEvents()}
                className="px-3 py-2 bg-red-500/20 hover:bg-red-500/30 text-red-300 rounded text-xs font-bold uppercase"
              >
                Limpar testes
              </button>
            </div>
