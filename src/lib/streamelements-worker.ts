...
function resolveEventType(eventName: string, eventData: Record<string, unknown>, rawData: Record<string, unknown>) {
  const candidates = [
    eventName,
    eventData?.listener,
    eventData?.type,
    (eventData?.event as Record<string, unknown> | undefined)?.listener,
    (eventData?.event as Record<string, unknown> | undefined)?.type,
    rawData?.listener,
    rawData?.type,
  ];

  for (const candidate of candidates) {
    const normalized = normalizeEventType(typeof candidate === 'string' ? candidate : undefined);
...
function extractActivityList(payload: unknown): Record<string, unknown>[] {
  if (Array.isArray(payload)) return payload as Record<string, unknown>[];
  const obj = payload as Record<string, unknown> | null;
  if (Array.isArray(obj?.docs)) return obj.docs as Record<string, unknown>[];
  if (Array.isArray(obj?.items)) return obj.items as Record<string, unknown>[];
  if (Array.isArray(obj?.activities)) return obj.activities as Record<string, unknown>[];
  return [];
}

function toNormalizedStreamElementsEvent(activity: Record<string, unknown>) {
  const eventPayload = (activity?.data || activity?.event || activity) as Record<string, unknown>;
  ...
  const inferredType = resolveEventType(
    String(activity?.type || activity?.listener || ''),
    activity,
    eventPayload,
  );
...
async function pollRecentActivities(config: StreamElementsConfig) {
  ...
  try {
    ...
    for (const activity of activities.reverse()) {
      ...
      if (rememberProcessedActivity(normalized.activityId == null ? null : String(normalized.activityId))) {
        continue;
      }
...
async function handleEvent(data: Record<string, unknown>) {
  ...
  const eventType = normalizeEventType(typeof data.type === 'string' ? data.type : undefined);
  const evtData = (data.data && typeof data.data === 'object'
    ? data.data
    : data) as Record<string, unknown>;
  const badges = Array.isArray(evtData.badges) ? evtData.badges as string[] : [];
  ...
  if (data.type === 'message' || data.type === 'chat') {
    ...
    const { error } = await supabase.from('twitch_chat_messages').insert({
      username: (evtData.username as string) || (evtData.nick as string) || 'unknown',
      display_name: (evtData.displayName as string) || (evtData.username as string) || 'Unknown',
      message: (evtData.text as string) || (evtData.message as string) || '',
      color: (evtData.color as string) || '#FFFFFF',
      is_subscriber: badges.includes('subscriber') || false,
      is_moderator: badges.includes('moderator') || evtData.role === 'moderator' || false,
    });
  ...
  if (['tip'].includes(eventType)) {
    console.log('[SE Worker] 🎉 Processing exclusive SE alert type:', eventType);
    ...
    if (evtError) {
      if (evtError.code === '23505') {
        console.log('[SE Worker] ℹ️ Duplicate tip ignored:', eventId);
      } else {
        console.error('[SE Worker] ❌ Error saving tip event:', evtError);
      }
    } else {
      console.log('[SE Worker] ✅ Tip event saved to streamelements_events');
    }
  } else if (['follower', 'subscriber', 'cheer', 'raid', 'host'].includes(eventType)) {
    console.log('[SE Worker] ⏭️ Skipping Twitch-native event from SE (EventSub owns it):', eventType);
  } else {
